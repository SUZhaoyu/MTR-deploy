...
def length_normalize(points, length, warning=False):
    ...
    if warning and actual_length != length:
        print("WARNING: the received data_utils length does not match the expected length.")
...
def feature_normalize(features, method):
    ...
    if method == 'L2':
        ...
        features = features
    elif type(method) is int or type(method) is float:
        features /= method
    else:
...
def convert_threejs_bbox(bboxes):
    ...
    threejs_bboxes = []
    category_dict = ["Pedd", "Lugg", "Troly", "Ignore"]

    for box in bboxes:
        if box[0] * box[1] * box[2] > 0:
            threejs_bbox = [0] * 9
            threejs_bbox[:7] = box[:7]
            threejs_bbox[-1] = category_dict[int(box[-1])]
...
def convert_threejs_bbox_with_prob(bboxes):
    assert len(bboxes) > 0
    for bbox in bboxes:
        w, l, h, x, y, z = bbox[:6]
        bbox[:6] = [l, h, w, y, z, x]
