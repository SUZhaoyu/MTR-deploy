...
nvcc src/iou3d_kernel.cu -o build/iou3d_kernel.cu.o -c ${TF_CFLAGS[@]} -D GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC
g++ -std=c++11 src/iou3d_kernel.cpp build/iou3d_kernel.cu.o -o build/iou3d_kernel.so -shared ${TF_CFLAGS[@]} -fPIC -lcudart ${TF_LFLAGS[@]} -L /usr/local/cuda/lib64/ -I /usr/local/cuda/include
