...
// params boxes_b: (M, 5) 
// params bev_overlap_area: (N, M)
REGISTER_OP("BoxesOverlapBev")
    .Input("input_boxes_a: float32")
    .Input("input_boxes_b: float32")
    .Output("output_bev_overlap_area: float32")
    // .Attr("npoints: float")
    // .Attr("batchSize: int")
    // .Attr("nFeatures: int")
...
void boxesOverlapBevGPUKernelLauncher(
    const int num_a, 
    const float *boxes_a,
    const int num_b,
    const float *boxes_b,
    float *bev_overlap_area);

class BoxesOverlapBev: public OpKernel {
public:
    explicit BoxesOverlapBev(OpKernelConstruction* context): OpKernel(context) {
    ...
    void Compute(OpKernelContext* context) override {
        ...
        OP_REQUIRES(context, input_boxes_a.dims()==2 && input_boxes_a.shape().dim_size(1)==5,
                    errors::InvalidArgument("BoxesOverlapBev expects boxes_a in shape: [M, 5]."));

        const Tensor& input_boxes_b = context->input(1);
        auto input_boxes_b_ptr = input_boxes_b.template flat<float>().data();
        OP_REQUIRES(context, input_boxes_b.dims()==2 && input_boxes_b.shape().dim_size(1)==5,
                    errors::InvalidArgument("BoxesOverlapBev expects boxes_b in shape: [N, 5]."));

        const int num_a = input_boxes_a.dim_size(0);
        const int num_b = input_boxes_b.dim_size(0);
        

        Tensor* output_bev_overlap_area = nullptr;
        auto output_bev_overlap_area_shape = TensorShape({num_a, num_b});
        OP_REQUIRES_OK(context, context->allocate_output(0, output_bev_overlap_area_shape, &output_bev_overlap_area));
        float* output_bev_overlap_area_ptr = output_bev_overlap_area->template flat<float>().data();

        // printf("1");
        boxesOverlapBevGPUKernelLauncher(
            num_a, 
            input_boxes_a_ptr,
            num_b,
            input_boxes_b_ptr,
            output_bev_overlap_area_ptr);
        // printf("1");
...
REGISTER_KERNEL_BUILDER(Name("BoxesOverlapBev").Device(DEVICE_GPU), BoxesOverlapBev);
...
void nms3dNormalGPUKernelLauncher(
    const int boxes_num, 
    const float nms_overlap_thresh, 
    const float *boxes, 
    unsigned long long * mask);

class Nms3d: public OpKernel {
public:
    explicit Nms3d(OpKernelConstruction* context): OpKernel(context) {
    ...
    void Compute(OpKernelContext* context) override {
        ...
        auto input_boxes_ptr = input_boxes.template flat<float>().data();
        OP_REQUIRES(context, input_boxes.dims()==2 && input_boxes.shape().dim_size(1)==7,
                    errors::InvalidArgument("Nms3d expects boxes_a in shape: [M, 7]."));
...
REGISTER_KERNEL_BUILDER(Name("Nms3d").Device(DEVICE_GPU), Nms3d);