...
public:
    explicit RoiFilterOp(OpKernelConstruction* context): OpKernel(context) {
        ...
        OP_REQUIRES_OK(context, context->GetAttr("instance_max", &instance_max));
    }
...
//
        ...
        for (int b=0; b<batch_size; b++) {
            for (int i=0; i<input_num_list_ptr[b]; i++) {
                ...
                if (instance_max > 0) {
                    if (input_roi_conf_ptr[id] >= conf_thres && output_num_list_ptr[b] < instance_max) {
