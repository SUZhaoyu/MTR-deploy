...
REGISTER_OP("LaRoiPoolingFastOp")
    ...
    .Attr("grid_buffer_resolution: list(float)")
...
public:
    ...
    void Compute(OpKernelContext* context) override {
        ...
        int grid_buffer_dim_w = (int)floor(dimension[0] / grid_buffer_resolution[0]);
        int grid_buffer_dim_l = (int)floor(dimension[1] / grid_buffer_resolution[1]);
        int grid_buffer_dim_h = (int)floor(dimension[2] / grid_buffer_resolution[2]);
        std::vector<int> grid_buffer_dimension {grid_buffer_dim_w, grid_buffer_dim_l, grid_buffer_dim_h};
...
//        cudaMemset(output_idx_ptr, -1, kernel_number*voxel_num*channels*sizeof(int));
        ...
        roi_pooling_gpu_launcher(batch_size, input_point_num, channels,
                                 roi_num, voxel_size, pooling_size, padding_value, grid_buffer_size,
                                 grid_buffer_resolution,
                                 grid_buffer_dimension,
                                 offset,
                                 input_coors_ptr,
                                 input_features_ptr,
                                 roi_attrs_ptr,
