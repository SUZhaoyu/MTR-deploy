...
__device__ inline float iou3d_normal(float const * const box_a, float const * const box_b, 
                                    const float* box_a_z, const float* box_b_z) {    
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
    // params: box_a_z (2) [z1min, z1max]
    // params: box_b_z (2) [z2min, z2max]
    float left = fmaxf(box_a[0], box_b[0]), right = fminf(box_a[2], box_b[2]);
    float top = fmaxf(box_a[1], box_b[1]), bottom = fminf(box_a[3], box_b[3]);
    float width = fmaxf(right - left, 0.f), height = fmaxf(bottom - top, 0.f);
    float s_overlap = width * height;
    float sa = (box_a[2] - box_a[0]) * (box_a[3] - box_a[1]);
    float sb = (box_b[2] - box_b[0]) * (box_b[3] - box_b[1]);
...
__global__ void nms3d_normal_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 7) [x, y, z, w, l ,h, ry] z-up coordinate system
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            ...
            float cur_iou_3d = iou3d_normal(&cur_box_tmp[0], &block_box_tmp[0], &cur_box_z_tmp[0], &block_box_z_tmp[0]);
...
void boxesOverlapBevGPUKernelLauncher(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
...
void nms3dNormalGPUKernelLauncher(const int boxes_num, const float nms_overlap_thresh, const float *boxes, unsigned long long * mask ){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
    nms3d_normal_kernel<<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes, mask);
}
